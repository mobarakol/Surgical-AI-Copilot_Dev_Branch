...
def parse_args():
    ...
    # Model save path
    parser.add_argument("--save_path", type=str, default="/SAN/medic/Surgical_LLM_Agent/11Respawn/New/Best-Models/Enhanced-Energy/3e-7/r128/10-epoch", help="Path to save the best model")
    # Other parameters
    ...
    parser.add_argument("--best_model_path", type=str, default="/SAN/medic/Surgical_LLM_Agent/11Respawn/New/Best-Models/Enhanced-Energy/3e-7/r128/10-epoch",
    ...
    parser.add_argument("--output_dir", type=str, default="/SAN/medic/Surgical_LLM_Agent/11Respawn/New/Results/Enhanced-Energy/3e-7/r128/10-epoch",
...
#---------------------------DEFT-Galore part-------------------------------------------
def fft_energy_projection(A: torch.Tensor,
                          k: int,
                          generator: Optional[torch.Generator] = None
                          ) -> torch.Tensor:
    """
    A : (m, n) real/float matrix
    k : target rank
    Return Q : (m, k) real orthogonal basis
    """
    m, n = A.shape
    # (1) FFT (complex)
...
class GaLoreProjector:
    ...
    def get_orthogonal_matrix(self, weights, rank: int, type: str, *, generator: Optional[torch.Generator] = None):

        """
        type ∈ {'left','right','full'}
        Return real orthogonal matrix/matrix pair
        """
        ...
        if type in {"right", "full"}:
            ...
            Q_right = fft_energy_projection(
                A.t(), rank, generator).t().contiguous()  # (k,n)
...
optimizer = DEFT_GaLoreAdamW(param_groups, lr=args.lr, weight_decay=args.weight_decay)
...
def train(model, train_loader, valid_loader, optimizer, criterion, num_epochs, save_path):
    ...
    for epoch in range(num_epochs):
        ...
        epoch_time = epoch_end_time - epoch_start_time  # Calculate the total time for the epoch
...
def compute_metrics(grouped_pred_prompts, grouped_ans_prompts):
    """
    Compute ROUGE, BLEU, and METEOR metrics using the Hugging Face evaluate library.
...
# ========================= Code 1: Evaluation Metric Extraction and Calculation =========================
...
def process_evaluation_files(output_dir):
    ...
    extract the values of each metric, calculate the average, and save it as average.txt and average.csv
    """
    all_values = {
        'rouge1': [],
        'rougeL': [],
    ...
    evaluation_files = []
    # Traverse all files ending with _evaluation.txt in output_dir
    ...
    with open(avg_csv_file, 'w', encoding='utf-8') as csv_out:
        ...
        # Multiply the average values by 100 to convert to percentage values (rounded to two decimal places), Matching_Accuracy is not multiplied by 100 because it is already a percentage
        values = [
            averages['bleu1'] * 100,
            averages['bleu2'] * 100,
            averages['bleu3'] * 100,
        ...
        # Write to CSV file, first write the header, then write the data row
